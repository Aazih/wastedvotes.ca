...
WHERE DISTRICT_ID IN (SELECT DISTRICT_ID from districts where PARLIAMENT_ID = (SELECT PARLIAMENT_ID FROM parliament where PARLIAMENT_TYPE_ID = (SELECT PARLIAMENT_TYPE_ID FROM parliament_type where PARLIAMENT_TYPE='Federal') AND PARLIAMENT_NUMBER=40))
