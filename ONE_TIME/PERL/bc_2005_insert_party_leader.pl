...
$sth = $dbh->prepare('SELECT PARLIAMENT_ID FROM parliament p, parliament_type pt WHERE p.PARLIAMENT_TYPE_ID = pt.PARLIAMENT_TYPE_ID and pt.PARLIAMENT_TYPE=? AND p.PARLIAMENT_NUMBER=?');
$sth->execute($parliament_type,$parliament_number)
...
open(LEADER_FILE, "../../Data/BC_2005_party_leader.txt");
while ($file_line = <LEADER_FILE>)
{
	...
	$sth = $dbh->prepare('SELECT PARTY_ID FROM party where PARTY_ENGLISH_NAME=? and PARTY_UNIQUE_NAME LIKE \'BC%\'');
	...
	$sth->finish();
	print "\tWorking with party_id $party_id for party_name $party_name, candidate_name $candidate_name, leader_number decided is $leader_number\n";
